    ...
    QMainWindow(parent)
{
    ...
    // 加载背景图片
    QPixmap backgroundImage(":/rsc/images/back.jpg");
    QPalette palette;
...
void Snake::moveSnake()
{
    ...
    if(change == 0){
        ...
        tempNode->label->setStyleSheet("background:blue");
        tempNode->label->show();
    }
    else if(change == 2){
        tempNode->label->setStyleSheet("background:white");
        tempNode->label->show();
    }
    else if(change == 3){
        tempNode->label->setStyleSheet("background:black");
        tempNode->label->show();
    }
    else if(change == 4){
        tempNode->label->setStyleSheet("background:red");
        tempNode->label->show();
    }
    else if(change == 5){
        Snakecolor s;
        QString color = s.changeColor();
        tempNode->label->setStyleSheet(color);
        tempNode->label->show();
    }
    //游戏通关
...
void Snake::victory()
{
    ...
    change = 0;
    QMessageBox msgBox;
    QMovie* movie = new QMovie(":/rs/images/victory.gif"); // 胜利动画的文件路径

    // 设置对话框的标题和图标
    msgBox.setWindowTitle("Victory!");
    msgBox.setIconPixmap(movie->currentPixmap());
    QString labelText = "<html><body>"
                        "<div align='center'><font size=\"4\">恭喜取得胜利</font></div>"
                        "</body></html>";
    ...
    animationLabel->setFixedSize(300,300);
    ...
    msgBox.layout()->addWidget(animationLabel);
